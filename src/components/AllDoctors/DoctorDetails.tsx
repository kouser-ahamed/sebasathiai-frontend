      ...
      <div className="mx-auto w-full max-w-6xl space-y-7 px-4 sm:px-6 lg:px-8">
        ...
        <section className="overflow-hidden rounded-[2rem] border border-[#F5CBCB] bg-white shadow-sm dark:border-[#41354A] dark:bg-[#2A2233]">
          <div className="grid items-stretch lg:grid-cols-[340px_1fr]">
            ...
            <div className="flex min-h-80 items-center justify-center overflow-hidden bg-gradient-to-br from-[#FBEFEF] via-[#FFE2E2] to-[#C5B3D3] p-3 sm:min-h-[420px] sm:p-4 md:min-h-[480px] lg:min-h-80 lg:p-0 dark:from-[#352B3D] dark:via-[#2A2233] dark:to-[#41354A]">
              {doctor.image ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img
                  src={doctor.image}
                  ...
                  className="h-full max-h-[480px] min-h-[296px] w-full rounded-2xl object-contain object-center sm:min-h-[388px] md:min-h-[448px] lg:max-h-none lg:min-h-80 lg:rounded-none lg:object-cover lg:object-top"
                />
              ) : (
                <div className="flex h-full min-h-[296px] w-full items-center justify-center rounded-2xl text-8xl font-black text-[#745D83] sm:min-h-[388px] md:min-h-[448px] lg:min-h-80 lg:rounded-none dark:text-[#F5CBCB]">
                  {initial}
                </div>
              )}
            </div>
            ...
            <div className="flex h-full flex-col p-6 sm:p-8">
              <div>
                <span className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-black text-emerald-700 dark:bg-emerald-950/25 dark:text-emerald-400">
                ...
                <p className="mt-2 flex items-center gap-2 text-base font-black text-[#745D83] dark:text-[#F5CBCB]">
                  <LuStethoscope className="size-5 shrink-0" />
                ...
                <div className="mt-4 flex flex-wrap items-center gap-3">
                  ...
                  <span className="text-sm text-slate-500 dark:text-[#A997AE]">
                    from {doctor.ratingCount} rating
                    {doctor.ratingCount === 1 ? "" : "s"}
                  </span>
                </div>

                <p className="mt-5 whitespace-pre-wrap text-sm leading-7 text-slate-600 dark:text-[#CDBFD0]">
                  {doctor.bio ||
                    "No professional biography has been provided."}
                </p>
              </div>

              {/* Full-width button always at the bottom */}

              <div className="mt-auto w-full pt-6">
                <AppointmentNowButton
                  doctorId={doctor.id}
