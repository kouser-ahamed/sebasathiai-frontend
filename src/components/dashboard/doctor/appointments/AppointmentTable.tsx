...
import { LuCircleCheck, LuEye, LuSettings2 } from "react-icons/lu";
...
interface AppointmentTableProps {
  ...
  onManage: (appointment: DoctorAppointment) => void;
        ...
        <table className="w-full min-w-[1150px] text-left">
          ...
          <tbody className="divide-y divide-[#F5CBCB] dark:divide-[#41354A]">
            {appointments.map((appointment) => {
              const initial =
                appointment.patientName.trim().charAt(0).toUpperCase() || "P";

              /*
               * Pending, approved and rejected
               * appointments can open Manage.
               * Completed appointments are final.
               */
              const canManage = appointment.status !== "completed";
                    ...
                    <div className="flex justify-end gap-2">
                      ...
                      {canComplete && (
                        <button
                          ...
                          className="inline-flex h-9 items-center gap-2 rounded-xl bg-emerald-600 px-3 text-xs font-black text-white transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-40"
                        >
